...
<h1 style="font-size:3.25rem;"> PUMP: Power Under Multiplicity Project </h1>


<h2 style="font-size:1.25rem;">Kristen Hunter<sup>1</sup>, Luke Miratrix<sup>1,2</sup>, <br>
Kristin Porter<sup>3</sup>, Zarni Htet <sup>3</sup></h2>

<h3 style="font-size:1rem;">National Cancer Institute<br>Statistical Adjustment for Multiplicity Workshop<br>2022</h3>

<p style="font-size:0.75rem">
Slides available: github.com/kristenbhunter/presentations/tree/master/NCI2022
</p>
...
- **Solution**: Easy-to-use software for calculating power, sample sample size, and minimum detectable effect size (MDES)
- Easy exploration of power over changes in assumed parameter values
...
- **Multilevel**: hierarchical structure, such as patients nested within hospitals nested within hospital systems
- Assumes frequentist **mixed effects** models

]

.pull-right[
<p style="float: left; font-size: 5pt; text-align: center; width: 100%; margin-right: 1%; margin-bottom: 2em"><img src="figures/pump.jpg" style="width: 80%; height: 400px"><br>Image by <a href="https://pixabay.com/users/947051-947051/?utm_source=link-attribution&amp;utm_medium=referral&amp;utm_campaign=image&amp;utm_content=1008977">Jan Steiner</a> from <a href="https://pixabay.com/?utm_source=link-attribution&amp;utm_medium=referral&amp;utm_campaign=image&amp;utm_content=1008977">Pixabay</a></p>
]
...
\begin{align*}
\underbrace{Y_{ijkm}}_{\text{Outcome}} &= \underbrace{\psi_{1,m}}_{\text{Tx effect}} \  \underbrace{T_{jk}}_{\text{Tx asst}}  \\
&+ \underbrace{\mu_{0,km}}_{\text{Level 2 Grand mean}} \\
&+ \sum_{r=1}^{g_{2,m}} \delta_{mr} \underbrace{X_{jkmr}}_{\text{Level 2 covariate}}  \\
&+ \sum_{p=1}^{g_{1,m}} \gamma_{mp} \underbrace{C_{ijkmp}}_{\text{Level 1 covariate}}  \\ 
&+ \underbrace{u_{0,jkm}}_{\text{Level 2 random effect}} \\
&+ \underbrace{r_{ijkm}}_{\text{Level 1 random effect}}
\end{align*}
...
- K: number of level 3 units (hospital systems)
- J: number of level 2 units (hospitals)
- nbar: number of level 1 units (patients)
...
Example: how many outcomes are assumed to have zero effect?

```{r, cache = TRUE, echo = TRUE}
gridZero <- update_grid( 
  pow,
  # vary parameter
  numZero = 0:4,
...
- Shiny app: [<span style="color:pink">mdrc.shinyapps.io/pump</span>](https://mdrc.shinyapps.io/pump/)
...
- Slides: [<span style="color:pink">github.com/kristenbhunter/presentations/tree/master/NCI2022</span>](https://github.com/kristenbhunter/presentations/tree/master/RAND2022)
